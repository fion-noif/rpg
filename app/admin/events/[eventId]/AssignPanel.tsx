...
//
// With JavaScript off the form still submits and the mechanic is still added and assigned;
// only the one-time link is lost, which "Rotate link" on the Mechanics tab recovers. That is
// the same tradeoff AddMechanicForm documents.
import { useActionState } from 'react';
import { assignMechanicAction } from './actions';
...
export function AssignPanel({
  ...
  /** Mechanics on this event not already assigned to this customer. Filtered by id upstream:
   *  names are not identity in this app (§23 Rule 1), so two people called Nia must not
   *  disappear from the list together. */
  mechanics: AssignableMechanic[];
  staff: StaffOption[];
}) {
  const [state, formAction, pending] = useActionState(assignMechanicAction, emptyAssignState);
  ...
  return (
    <div className="admin-assign-body">
      ...
      {/* Renders the link box when this assignment also created the mechanic, the error copy

...
/**
 * Why a token did not resolve, so the one screen a locked-out mechanic is looking at can say
...
export type TokenResolution =
  | { ok: true; mechanic: MechanicSession; expiresAt: Date }
 ...
 * token, so a manager who pushes a long weekend's end date back revives every mechanic's link
...
export async function resolveToken(token: string | undefined): Promise<TokenResolution> {
  ...
  // are NULLed rather than flagged, and the synthetic per-event manager mechanic never has
  ...
  const rows = await q<MechanicSession & { expires_at: string; expired: boolean }>(
  ...
  const { expires_at, expired, ...mechanic } = row;
  return { ok: true, mechanic, expiresAt: new Date(expires_at) };
}

/**
 * Session lookup for callers that only need "is this a mechanic, yes or no" — the two API
 ...
 */
export async function mechanicByToken(token: string | undefined): Promise<MechanicSession | undefined> {
  const resolution = await resolveToken(token);
  return resolution.ok ? resolution.mechanic : undefined;
}

/**
 * Mints a fresh magic-link token for a mechanic, storing only its hash and clearing any
 * previous revocation. Returns the plaintext, which is the one and only time it exists —
 * losing it means rotating again.
 */
export async function issueToken(mechanicId: number, client: Queryable = pool): Promise<string> {
  const token = newToken();
  await client.query('UPDATE mechanics SET token_hash = $1, token_revoked_at = NULL WHERE id = $2', [
    hashToken(token),
    mechanicId,
  ]);
  return token;
...
/**
 * Destroys every mechanic credential for an event (design doc §8 least privilege): closing a
 * weekend must make the links *gone*, not merely ignored, so the hash is dropped and the
 * revocation stamped. The synthetic admin mechanic is skipped — it has no token to begin
...
export async function revokeEventTokens(eventId: number, client: Queryable = pool): Promise<number> {
  const res = await client.query(
    `UPDATE mechanics SET token_hash = NULL, token_revoked_at = now()
